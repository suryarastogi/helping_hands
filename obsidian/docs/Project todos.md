...
3. **Autodocs** — complete (MkDocs Material + mkdocstrings, 38 API pages, GitHub Pages)
...
- Docstrings: Google-style on all public methods and all private helpers; `_TwoPhaseCLIHand` and all 4 CLI hands have complete docstring coverage (base timing accessors, override methods, `_invoke_backend`, `_pr_description_cmd`, `_build_failure_message`, `_command_not_found_message`), `AIProvider` (including all `_build_inner`/`_complete_impl`), skills runners, meta/tools helpers, `_build_agent` methods, and `GitHubClient` context manager fully documented.
