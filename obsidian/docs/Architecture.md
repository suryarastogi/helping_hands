...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. Cron-scheduled builds are managed via RedBeat (`server/schedules.py`) with CRUD endpoints. The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
...
5. Open PR with generated summary body. Rich PR title/body can be generated via `pr_description.py` using a CLI tool (e.g. `claude -p`, `gemini -p`) from the git diff; falls back gracefully when unavailable.
...
1. `goose` CLI installed and on `PATH`.
2. Provider/model are injected via `GOOSE_PROVIDER`/`GOOSE_MODEL` env vars, auto-derived from `HELPING_HANDS_MODEL` (default: `ollama` + `llama3.2:latest`).
3. `GH_TOKEN` or `GITHUB_TOKEN` always required; runtime mirrors whichever is set to both variables.
4. `goose run` auto-injects `--with-builtin developer` if missing.
5. Local GitHub auth fallback is disabled for Goose runs.
...
3. Non-interactive runs default to `--approval-mode auto_edit`.
4. If a requested model is unavailable, `geminicli` retries once without `--model`.

## Cron-scheduled tasks

App mode supports cron-scheduled recurring build tasks via `server/schedules.py`:

- Uses RedBeat for Redis-backed schedule persistence in Celery Beat.
- Provides CRUD operations (`create_schedule`, `get_schedule`, `list_schedules`, `update_schedule`, `delete_schedule`, `enable_schedule`, `disable_schedule`).
- Supports standard cron expressions plus common presets (hourly, daily, weekly, etc.).
- Manual trigger support via `trigger_now`.
