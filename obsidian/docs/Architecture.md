...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. Cron-scheduled tasks are backed by RedBeat/Redis with CRUD API endpoints and a scheduling UI. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
...
3. **Hand backend** (`Hand` + implementations) — Common protocol with `E2EHand`, `LangGraphHand`, `AtomicHand`, basic iterative hands, and fully implemented CLI hands (`codexcli`, `claudecodecli`, `goose`, `geminicli`).
   - Current code shape is a package module: `lib/hands/v1/hand/` (`base.py`, `langgraph.py`, `atomic.py`, `iterative.py`, `e2e.py`, `pr_description.py`, `cli/*.py`, `placeholders.py` compatibility shim, `__init__.py` export surface).
...
8. **Scheduling** (`server/schedules.py`) — Cron-scheduled build tasks backed by celery-redbeat/Redis with CRUD API, preset expressions, and croniter validation.
9. **PR description generation** (`lib/hands/v1/hand/pr_description.py`) — CLI-tool-assisted PR title/body generation from git diffs during hand finalization.
10. **Default prompts** (`lib/default_prompts.py`) — Shared smoke-test and default prompt text used by CLI and app entry points.
11. **Entry points** — CLI, FastAPI app, and MCP server orchestrate calls to the same core.
...
3. `GH_TOKEN` or `GITHUB_TOKEN` always required.
4. `GOOSE_PROVIDER`/`GOOSE_MODEL` are auto-derived from `HELPING_HANDS_MODEL`.
5. The backend auto-adds `--with-builtin developer` for `goose run` commands if missing.
6. Runtime mirrors the available token into both `GH_TOKEN` and `GITHUB_TOKEN`.
...
6. Default idle timeout is 900s for all CLI backends.
...
## Data flow (cron scheduling)

```
Admin → FastAPI /schedules → RedBeat scheduler entries in Redis
                              ↓
                      Celery Beat reads RedBeat entries
                              ↓
                      fires build_feature task on schedule
                              ↓
              same worker execution path as manual /build
```

CRUD endpoints: `POST /schedules`, `GET /schedules`, `GET /schedules/{id}`,
