...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. Cron-scheduled submission tasks are supported via `/schedules` CRUD endpoints backed by RedBeat. The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
...
App-mode foundations are present (server, worker, broker/backend wiring) with cron-scheduled submission tasks via RedBeat.
...
3. **Hand backend** (`Hand` + implementations) — Common protocol with `E2EHand`, `LangGraphHand`, `AtomicHand`, basic iterative hands (`BasicLangGraphHand`, `BasicAtomicHand`), plus CLI hands (`CodexCLIHand`, `ClaudeCodeHand`, `GooseCLIHand`, `GeminiCLIHand`).
...
3. Always requires `GH_TOKEN` or `GITHUB_TOKEN`; runtime mirrors the available token to both variables.
4. `GOOSE_PROVIDER`/`GOOSE_MODEL` are auto-derived from `HELPING_HANDS_MODEL` (fallback: `ollama` + `llama3.2:latest`).
5. Backend auto-adds `--with-builtin developer` for `goose run` commands so file editing tools are available.
...
2. `GEMINI_API_KEY` is always required (no native-CLI-auth toggle).
3. `geminicli` injects `--approval-mode auto_edit` by default for non-interactive scripted runs.
4. If a requested model is unavailable, `geminicli` retries once without `--model` so Gemini CLI picks a default.
5. Override command via `HELPING_HANDS_GEMINI_CLI_CMD`.
6. Default idle timeout is 900s for all CLI backends (override with `HELPING_HANDS_CLI_IDLE_TIMEOUT_SECONDS`).
