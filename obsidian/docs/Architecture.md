...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. Cron-scheduled builds are supported via RedBeat scheduler with CRUD schedule management API (`/schedules`). The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
- **MCP mode (implemented baseline)** — MCP server exposes tools for repo indexing, build enqueue/status, filesystem operations (`read_file`, `write_file`, `mkdir`, `path_exists`), code execution (`run_python_code`, `run_python_script`, `run_bash_script`), web tools (`web_search`, `web_browse`), and config inspection.
...
2. `GH_TOKEN` or `GITHUB_TOKEN` always required (runtime mirrors to both).
3. Provider/model auto-injected via `GOOSE_PROVIDER`/`GOOSE_MODEL` derived from
   `HELPING_HANDS_MODEL` (default: `ollama` + `llama3.2:latest`).
4. `--with-builtin developer` is included by default for file editing tools.
...
4. If a requested model is unavailable, retries once without `--model`.
