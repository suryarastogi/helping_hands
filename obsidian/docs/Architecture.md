...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. Cron-scheduled tasks are managed via `/schedules` endpoints (RedBeat scheduler). The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
- **MCP mode (implemented baseline)** — MCP server exposes tools for repo indexing, build enqueue/status, filesystem operations (`read_file`, `write_file`, `mkdir`, `path_exists`), command execution (`run_python_code`, `run_python_script`, `run_bash_script`), web tools (`web_search`, `web_browse`), and config inspection.
...
3. **Hand backend** (`Hand` + implementations) — Common protocol with `E2EHand`, `LangGraphHand`, `AtomicHand`, basic iterative hands (`BasicLangGraphHand`, `BasicAtomicHand` in `iterative.py`), and CLI hands (`CodexCLIHand`, `ClaudeCodeHand`, `GooseCLIHand`, `GeminiCLIHand` in `cli/`).
   - Current code shape is a package module: `lib/hands/v1/hand/` (`base.py`, `langgraph.py`, `atomic.py`, `iterative.py`, `e2e.py`, `pr_description.py`, `model_provider.py`, `cli/*.py`, `placeholders.py` compatibility shim, `__init__.py` export surface).
...
6. **System tools layer** (`lib.meta.tools`) — Shared path-safe operations consumed by iterative hands and MCP tools:
   - `filesystem` — file read/write/mkdir with path resolution/validation.
   - `command` — sandboxed Python/Bash command execution (`run_python_code`, `run_python_script`, `run_bash_script`).
   - `web` — web search and URL browsing helpers.
   **Skills** (`lib.meta.skills`) — runtime-selectable prompt bundles injected into iterative hands via `--skills`.
...
2. `GH_TOKEN` or `GITHUB_TOKEN` always required; runtime mirrors one to both.
3. `GOOSE_PROVIDER`/`GOOSE_MODEL` auto-derived from `HELPING_HANDS_MODEL` (default: `ollama` + `llama3.2:latest`).
4. `goose run` commands auto-inject `--with-builtin developer` for file editing tools.
5. For remote Ollama instances, set `OLLAMA_HOST`.
6. Local GitHub auth fallback is intentionally disabled for Goose runs.
7. App-mode worker runtime must have `goose` installed; the included Dockerfile installs Goose CLI.
...
5. Override command via `HELPING_HANDS_GEMINI_CLI_CMD`.

## Scheduled tasks (cron)

App mode supports cron-scheduled submission tasks via RedBeat:

1. `/schedules` CRUD endpoints manage named cron schedules.
2. Each schedule stores backend, model, prompt, and execution options.
3. Celery Beat + RedBeat fires tasks on cron expression match.
4. Manual trigger via `POST /schedules/{id}/trigger`.
5. `/cron-presets` returns common cron expression templates.
...
User → CLI
  ...
  cli backend:   Config → CLI*Hand two-phase subprocess → optional final PR
