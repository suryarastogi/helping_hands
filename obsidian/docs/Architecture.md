...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. Cron-scheduled submissions are managed via RedBeat (`server/schedules.py`) with CRUD endpoints and preset cron expressions. The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
...
App-mode foundations are present (server, worker, broker/backend wiring), and cron-scheduled task submission is now supported via RedBeat.
...
3. **Hand backend** (`Hand` + implementations) — Common protocol with `E2EHand`, `LangGraphHand`, `AtomicHand`, basic iterative hands, and CLI hands (`CodexCLIHand`, `ClaudeCodeHand`, `GooseCLIHand`, `GeminiCLIHand`).
   - Current code shape is a package module: `lib/hands/v1/hand/` (`base.py`, `langgraph.py`, `atomic.py`, `iterative.py`, `e2e.py`, `model_provider.py`, `pr_description.py`, `cli/*.py`, `placeholders.py` compatibility shim, `__init__.py` export surface).
...
6. **System tools layer** (`lib.meta.tools`) — Shared path-safe file operations (`filesystem`), code/script execution (`command`), and web search/browse helpers (`web`) consumed by iterative hands and MCP tools.
...
2. Provider-dependent auth: `OPENAI_API_KEY` (openai), `ANTHROPIC_API_KEY` (anthropic), `GOOGLE_API_KEY` (google), or `OLLAMA_HOST`/`OLLAMA_API_KEY` (ollama, default).
3. `GH_TOKEN` or `GITHUB_TOKEN` always required; runtime mirrors available token to both variables.
4. `GOOSE_PROVIDER`/`GOOSE_MODEL` auto-derived from `HELPING_HANDS_MODEL` (default: `ollama` + `llama3.2:latest`).
5. `goose run` auto-injects `--with-builtin developer` so file editing tools are available.
...
4. If a requested Gemini model is unavailable, the backend retries once without `--model`.
