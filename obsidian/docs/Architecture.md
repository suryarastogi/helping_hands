...
- **App mode (implemented)** — FastAPI + Celery integration supports `e2e`, `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, and `geminicli`. `/build` enqueues `build_feature`; `/tasks/{task_id}` returns JSON status/result; `/monitor/{task_id}` provides an auto-refresh no-JS monitor page. Cron-scheduled builds use RedBeat scheduler with CRUD management via `schedules.py`. The UI defaults prompt text to a smoke-test `README.md` updater that exercises `@@READ`, `@@FILE`, and (when enabled) `python.run_code`, `python.run_script`, `bash.run_script`, `web.search`, and `web.browse`. Execution and web tools are opt-in (`enable_execution`, `enable_web`). Monitor cells remain fixed dimensions with in-cell scrolling.
...
App-mode foundations are production-ready (server, worker, broker/backend wiring, cron scheduling via RedBeat).
...
6. **System tools layer** (`lib.meta.tools`) — Three tool modules consumed by hands and MCP:
   - `filesystem` — path-safe file operations (`read_text_file`, `write_text_file`, `mkdir_path`, path resolution/validation)
   - `command` — repo-aware command execution (`run_python_code`, `run_python_script`, `run_bash_script`)
   - `web` — web browsing and search (`search_web`, `browse_url`)
...
2. `GH_TOKEN` or `GITHUB_TOKEN` present (always required; runtime mirrors to both vars).
3. Provider/model auto-derived from `HELPING_HANDS_MODEL` (default: `ollama` + `llama3.2:latest`).
4. `--with-builtin developer` auto-injected for `goose run` commands so file editing tools are available.
5. `goose configure` is not required; env vars drive all configuration.
...
4. If Gemini rejects a deprecated/unavailable model, backend retries once without `--model`.
