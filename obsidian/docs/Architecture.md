...
3. **Hand backend** (`Hand` + implementations) — Common protocol with `E2EHand`, `LangGraphHand`, `AtomicHand`, basic iterative hands, plus CLI hands (`CodexCLIHand`, `ClaudeCodeHand`, `GooseCLIHand`, `GeminiCLIHand`).
...
2. Provider/model auto-derived from `HELPING_HANDS_MODEL` (default: `ollama` +
   `llama3.2:latest`); `goose configure` not required.
3. `GH_TOKEN` or `GITHUB_TOKEN` always required; runtime mirrors to both vars.
4. `goose run` auto-adds `--with-builtin developer` for file editing/shell tools.
5. Override command via `HELPING_HANDS_GOOSE_CLI_CMD`.
...
3. `GITHUB_TOKEN` or `GH_TOKEN` present if final commit/push/PR is expected.
4. Default approval mode: `--approval-mode auto_edit` (override via
   `HELPING_HANDS_GEMINI_CLI_CMD` with explicit `--approval-mode`).
