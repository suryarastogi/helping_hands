...
(fallback: `ollama` + `llama3.2:latest`). The backend auto-adds
`--with-builtin developer` and always requires `GH_TOKEN`/`GITHUB_TOKEN`.

For `geminicli`, `GEMINI_API_KEY` is always required (no native-CLI-auth toggle).
The backend injects `--approval-mode auto_edit` by default for non-interactive
runs and retries once without `--model` if Gemini rejects a deprecated model.
