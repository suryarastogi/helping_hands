...
`--with-builtin developer` is included by default. `GH_TOKEN`/`GITHUB_TOKEN` is
always required and mirrored to both env vars.
...
once without `--model`. `GEMINI_API_KEY` is always required.
...
This keeps type checks actionable while optional-backend imports stabilize.
