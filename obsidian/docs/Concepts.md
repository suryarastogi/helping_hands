...
(default: `ollama` + `llama3.2:latest`), `GH_TOKEN`/`GITHUB_TOKEN` is always
required, and `goose run` auto-adds `--with-builtin developer`.

For `geminicli`, `GEMINI_API_KEY` is always required (no native-CLI-auth
toggle), and the backend injects `--approval-mode auto_edit` by default. If
Gemini rejects a deprecated model, the backend retries once without `--model`.
