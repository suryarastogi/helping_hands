...
# ---------------------------------------------------------------------------
# Cross-module import consistency — command-not-found (rebuild hint)
# ---------------------------------------------------------------------------


class TestDockerRebuildHintCrossModuleSync:
    """CLI hands importing _DOCKER_REBUILD_HINT_TEMPLATE use the same object."""
    ...
    def test_goose_uses_same_object(self) -> None:
        src = inspect.getsource(
            __import__(
                "helping_hands.lib.hands.v1.hand.cli.goose",
                fromlist=["GooseCLIHand"],
            )
        )
        assert "_DOCKER_REBUILD_HINT_TEMPLATE" in src
