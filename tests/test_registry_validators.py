...
class TestParseStrList:
    def test_valid_list(self) -> None:
        assert parse_str_list({"args": ["a", "b"]}, key="args") == ["a", "b"]

    def test_empty_list(self) -> None:
        assert parse_str_list({"args": []}, key="args") == []

    def test_missing_key_returns_empty(self) -> None:
        assert parse_str_list({}, key="args") == []

    def test_none_returns_empty(self) -> None:
        assert parse_str_list({"args": None}, key="args") == []

    def test_rejects_non_list(self) -> None:
        with pytest.raises(ValueError, match="list of strings"):
            parse_str_list({"args": "not-a-list"}, key="args")

    def test_rejects_non_string_items(self) -> None:
        with pytest.raises(ValueError, match="only strings"):
            parse_str_list({"args": [1, 2]}, key="args")

    def test_rejects_empty_string_items(self) -> None:
        with pytest.raises(ValueError, match="empty or whitespace-only"):
            parse_str_list({"args": ["valid", ""]}, key="args")
