...
- **BasicLangGraphHand** (`iterative.py`) — iterative LangGraph loop with `@@READ`/`@@FILE` in-model file operations (requires `--extra langchain`)
- **BasicAtomicHand** (`iterative.py`) — iterative Atomic Agents loop with `@@READ`/`@@FILE` (requires `--extra atomic`)
