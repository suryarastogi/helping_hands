...
- **_BasicIterativeHand** (`iterative.py`) — shared base for loop-based hands with `@@READ`/`@@FILE` in-model file operations
- **BasicLangGraphHand** (`iterative.py`) — iterative LangGraph backend (requires `--extra langchain`)
- **BasicAtomicHand** (`iterative.py`) — iterative Atomic Agents backend (requires `--extra atomic`)
- **LangGraphHand** (`langgraph.py`) — direct LangGraph agent (requires `--extra langchain`)
- **AtomicHand** (`atomic.py`) — direct Atomic Agents agent (requires `--extra atomic`)
