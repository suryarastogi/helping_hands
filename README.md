...
`helping_hands` is a Python tool that takes a git repository as input, understands its structure and conventions, and collaborates with you to add features, fix bugs, and evolve the codebase using AI. It supports multiple AI backends and can run as a **CLI** for interactive use or as a **server** with background workers for queued and scheduled builds.

### Modes

- **CLI mode** (default) — Run `helping-hands <repo>` to index a local repo or auto-clone a GitHub `owner/repo`. Supports iterative backends (`basic-langgraph`, `basic-atomic`, `basic-agent`) and external CLI backends (`codexcli`, `claudecodecli`, `docker-sandbox-claude`, `goose`, `geminicli`, `opencodecli`, `devincli`).
- **App mode** — FastAPI server + Celery/Redis/Postgres worker stack for asynchronous and scheduled builds, with a React frontend and queue monitoring via Flower. See [docs/app-mode.md](docs/app-mode.md).
...
- **[Backend Reference](docs/backends.md)** — Environment variables, auth setup, and usage notes for every backend (Codex, Claude Code, Docker Sandbox Claude, Goose, Gemini, OpenCode, Devin)
- **[App Mode & Scheduling](docs/app-mode.md)** — Running the server stack, triggering builds via API, local Celery setup, cron/interval scheduling, and PR number semantics
- **[Development & Contributing](docs/development.md)** — Install, lint, test, coverage, pre-commit hooks, frontend checks, configuration reference
