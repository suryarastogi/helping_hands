...
1. **Empty command list validation in `_run_command()`** (`command.py`) — Added `ValueError` guard for empty command lists, preventing `IndexError` in `FileNotFoundError` handler at `command[0]`.

2. **Maximum timeout cap for web tools** (`web.py`) — Added `_MAX_WEB_TIMEOUT_S = 300` constant with warning log when clamping, consistent with `_MAX_GIT_TIMEOUT` pattern in `github.py`. Applied to both `search_web()` and `browse_url()`.

3. **OSError handling in `_get_diff`/`_get_uncommitted_diff`** (`pr_description.py`) — Added `OSError` catch with debug logging to all four `subprocess.run()` calls (both functions), handling permission denied, broken symlinks, etc. gracefully.
...
- [x] Add tests (15 new: 3 command validation, 8 web timeout cap, 4 OSError handling)
