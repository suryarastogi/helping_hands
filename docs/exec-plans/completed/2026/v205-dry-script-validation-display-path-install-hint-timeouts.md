...
- [x] Extract `_validate_script_path` in command.py — shared helper replaces
  duplicated 5-line script-not-found / is-a-directory validation in both
  `run_python_script` and `run_bash_script`
- [x] Extract `_display_path` in filesystem.py — shared helper replaces 4×
  inline `target.relative_to(root).as_posix()` calls across `read_text_file`,
  `write_text_file`, and `mkdir_path`
- [x] DRY `install_hint` in AI provider error messages — all 5 provider
  `_build_inner()` methods now use `self.install_hint` in the RuntimeError
  message instead of hardcoding the install command string
- [x] DRY timeout constants to `server/constants.py` — `KEYCHAIN_TIMEOUT_S`
...
- [x] Add tests covering all changes
