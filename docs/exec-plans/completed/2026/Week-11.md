...
## Mar 12 (cont.) — Claude CLI, PR description, and CLI failure output constant extraction (v139)

Extracted hardcoded magic numbers to module-level constants across six files. In `cli/claude.py`: `_TEXT_PREVIEW_MAX_LENGTH` (200), `_TOOL_RESULT_PREVIEW_MAX_LENGTH` (150), `_COMMAND_PREVIEW_MAX_LENGTH` (80), `_FAILURE_OUTPUT_TAIL_LENGTH` (2000). In `pr_description.py`: `_PR_SUMMARY_TRUNCATION_LENGTH` (2000), `_COMMIT_SUMMARY_TRUNCATION_LENGTH` (1000), `_PROMPT_CONTEXT_LENGTH` (500), `_PR_ERROR_TAIL_LENGTH` (500), `_COMMIT_ERROR_TAIL_LENGTH` (300), `_COMMIT_MSG_MAX_LENGTH` (72). DRY `_FAILURE_OUTPUT_TAIL_LENGTH` (2000) across `cli/codex.py`, `cli/gemini.py`, `cli/opencode.py` with cross-module sync test. Imported `_FILE_LIST_PREVIEW_LIMIT` from `base.py` in `cli/base.py` to replace hardcoded `[:200]`. **3436 passing tests (+29 new constant value/type/sign/sync invariant tests), 37 skipped.**

## Mar 12 (cont.) — Remaining magic numbers, GitHub PR validation, truncation safety (v140)

Extracted remaining hardcoded magic numbers: `_APPLY_CHANGES_TRUNCATION_LIMIT` (2000) and `_STREAM_READ_BUFFER_SIZE` (1024) in `cli/base.py`, `_DEFAULT_MAX_TOKENS` (1024) in `anthropic.py`. `_STREAM_READ_BUFFER_SIZE` is also imported and used in `docker_sandbox_claude.py`. Added PR number validation (`number > 0`) to `get_pr()` and `update_pr_body()` in `github.py`, and limit validation (`limit > 0`) to `list_prs()`. Added `limit > 0` validation to `_truncate_diff()` in `pr_description.py`. **3456 passing tests (+20 new), 37 skipped.**

## Mar 12 (cont.) — E2E marker constant, CLI PR number validation, Celery timeout constants (v141)

Extracted `_E2E_MARKER_FILE = "HELPING_HANDS_E2E.md"` module-level constant in `e2e.py` replacing hardcoded inline string. Added `--pr-number` positive integer validation in `cli/main.py` — zero/negative values now produce a clear error message and exit before reaching the E2E hand. Extracted `_KEYCHAIN_TIMEOUT_S` (5) and `_DB_CONNECT_TIMEOUT_S` (5) module-level constants in `celery_app.py` replacing hardcoded timeout values in the Keychain subprocess call and psycopg2 connection, matching the pattern from `server/app.py` (v137). Added cross-module sync test to verify `_KEYCHAIN_TIMEOUT_S` stays in sync between `celery_app.py` and `app.py`. **3464 passing tests (+8 new), 44 skipped.**

---

## Mar 12 (cont.) — Remaining magic numbers and repo PermissionError handling (v142)

Extracted remaining hardcoded magic numbers to module-level constants: `_HTTP_ERROR_BODY_PREVIEW_LENGTH` (200) and `_USAGE_DATA_PREVIEW_LENGTH` (300) in `server/app.py` for HTTP error body and usage data preview truncation in `_fetch_claude_usage()`. `_HOOK_ERROR_TRUNCATION_LIMIT` (3000) and `_GIT_REF_DISPLAY_LENGTH` (8) in `cli/base.py` for hook error output truncation in `_build_hook_fix_prompt()` and git ref display in `_poll_ci_checks()`. `_SANDBOX_NAME_MAX_LENGTH` (30) and `_SANDBOX_UUID_HEX_LENGTH` (8) in `docker_sandbox_claude.py` for sandbox name generation in `_resolve_sandbox_name()`. Added `PermissionError` handling to `RepoIndex.from_path()` in `repo.py` — `rglob()` no longer crashes on directories with restricted permissions; instead logs a warning and returns an empty file list. **3480 passing tests (+18 new passed, 8 skipped without fastapi), 52 skipped.**

---

## Mar 12 (cont.) — Redis error handling and constant extraction (v143)

Added Redis error handling to `ScheduleManager` in `schedules.py`: `_save_meta()` now catches Redis write failures, logs a warning, and raises `RuntimeError` (previously silent data loss). `_delete_meta()` catches Redis delete failures with warning log and graceful swallow (consistent with `_load_meta` pattern from v128). `_list_meta_keys()` catches Redis failures with warning log and returns empty list (prevents `list_schedules()` crash). Extracted `_SCHEDULE_ID_HEX_LENGTH = 12` module-level constant from hardcoded `[:12]` in `generate_schedule_id()`. Extracted `_OLLAMA_DEFAULT_HOST = "http://localhost:11434"` module-level constant in `goose.py` from hardcoded fallback in `_resolve_ollama_host()`. **3487 passing tests (+7 new), 52 skipped.**

---

## Mar 12 (cont.) — MCP index limit, JWT prefix constant, E2E UUID reuse (v144)

Extracted `_INDEX_FILES_LIMIT = 200` module-level constant in `mcp_server.py` from hardcoded `idx.files[:200]` in `index_repo()`. Extracted `_JWT_TOKEN_PREFIX = "ey"` module-level constant in both `server/app.py` (`_get_claude_oauth_token()`) and `server/celery_app.py` (`log_claude_usage()`) from duplicated JWT detection heuristic, with cross-module sync test. Imported `_UUID_HEX_LENGTH` from `base.py` in `e2e.py` to replace hardcoded `[:8]` in branch naming, maintaining DRY consistency. **3494 passing tests (+7 new), 60 skipped.**

---

## Mar 12 (cont.) — Keychain constants, utilization type guard, decode safety (v145)

Extracted keychain constants (`_KEYCHAIN_SERVICE_NAME = "Claude Code-credentials"`, `_KEYCHAIN_OAUTH_KEY = "claudeAiOauth"`, `_KEYCHAIN_ACCESS_TOKEN_KEY = "accessToken"`) to module-level in both `server/app.py` and `server/celery_app.py`, replacing hardcoded magic strings in `_get_claude_oauth_token()` and `log_claude_usage()` with cross-module sync tests. Hardened `_fetch_claude_usage()` utilization parsing: replaced `is not None` check with `isinstance(val, (int, float))` type guard to prevent `TypeError` on non-numeric API responses (string, list, dict values now gracefully skipped instead of crashing `round()`). Added `decode(errors="replace")` to API response decoding in `_fetch_claude_usage()` for both the success path and HTTP error body, preventing `UnicodeDecodeError` on non-UTF-8 responses. **3494 passing tests (+20 new, all skipped without fastapi/celery), 80 skipped.**
...
## Mar 13 — CI fix loop resilience and prompt quality (v147)

Added exception resilience to `_poll_ci_checks()` in `cli/base.py`: `gh.get_check_runs()` calls within the poll loop and the post-deadline final call are now wrapped in try-except, logging warnings and retrying on transient failures instead of crashing the entire CI fix loop. Post-deadline failures return a safe fallback dict (`{"conclusion": "pending", "total_count": 0}`). Cleaned up `_build_ci_fix_prompt()` URL formatting: empty `html_url` values no longer produce empty parentheses — lines now show `name: conclusion` without `()` when no URL is available. Added `logger.debug` with `exc_info=True` to the `_ci_fix_loop` broad `except Exception` handler, consistent with the project convention (v129) of always logging stack traces before suppressing exceptions. **3553 passing tests (+11 new), 80 skipped.**

---

**Week summary:** Systematic hardening across the codebase. Replaced all remaining `assert` statements in production code with explicit guards. Added debug logging to all silent exception handlers (including the final two bare re-raises in atomic.py/iterative.py in v129). Expanded Claude CLI tool summarization. Consolidated validators via mixin extraction. Added input validation to MCP server tools and server request models. Hardened git subprocess operations with timeout protection and input format validation. Added type annotations and test coverage for previously untested helpers. Added empty-string rejection to server request models and mutual exclusivity validation to bash script runner. Hardened AI provider message normalization and model validation. Added comprehensive test coverage for GooseCLIHand._build_subprocess_env (v129). Added defensive `.get()` for CI response handling and frontend form validation (v130). Added network error handling for web tools, clone depth validation, and Google provider empty messages guard (v131). Fixed CLI model None guard and added worker capacity + LangGraph test coverage (v132). Added warning logging for silent env var fallbacks, git-not-found error handling, and write_text_file OSError wrapping (v133). Hardened `_parse_str_list` with empty/whitespace rejection, added `_load_env_files` tilde expansion test, added `_collect_celery_current_tasks` direct test coverage (v134). Extracted hardcoded magic numbers to module-level constants in `mcp_server.py` and `celery_app.py` with tests (v135). Gitignore cleanup for Playwright test artifacts and commit message quality hardening with `_is_trivial_message()` rejection (v136). Extracted health-check timeout constants and DRY Anthropic API constants in `server/app.py` and `server/celery_app.py` (v137). Extracted hardcoded magic numbers to module-level constants in Hand base, CLI base, and CLI main (v138). Extracted preview/truncation constants in Claude CLI, PR description, and DRY failure output tail across CLI hands (v139). Extracted remaining magic numbers (apply-changes truncation, stream buffer, Anthropic max_tokens), added GitHub PR number/limit validation, and truncation limit safety guard (v140). Extracted E2E marker filename constant, added CLI `--pr-number` positive validation, and extracted Celery timeout constants (v141). Extracted remaining preview/display/naming magic numbers in server/app.py, cli/base.py, and docker_sandbox_claude.py, added PermissionError handling to repo.py `rglob()` (v142). Added Redis write/delete/list error handling to `ScheduleManager` in schedules.py, extracted `_SCHEDULE_ID_HEX_LENGTH` and `_OLLAMA_DEFAULT_HOST` constants (v143). Extracted MCP index file limit, JWT token prefix (DRY across app.py/celery_app.py), and reused `_UUID_HEX_LENGTH` in e2e.py (v144). Extracted keychain constants (service name, OAuth key, access token key) to module-level in both app.py and celery_app.py, added utilization numeric type guard and decode safety in `_fetch_claude_usage()` (v145). Commit message quality improvements: `_truncate_text()` truncation indicators for prompt/summary context and `_infer_commit_type()` smart type inference replacing hardcoded `"feat:"` prefix (v146). CI fix loop resilience: `_poll_ci_checks` API exception handling with retry, `_build_ci_fix_prompt` URL formatting cleanup, `_ci_fix_loop` exception debug logging (v147).
