...
| `lib/hands/v1/hand/iterative.py` (iteration helpers) | Excellent (79% -> 92%) | Maintained | `_build_iteration_prompt`, `_execution_tools_enabled`/`_web_tools_enabled`, `_tool_instructions`, `BasicLangGraphHand._result_content`, `BasicAtomicHand._extract_message` added in v20; `_execute_read_requests` error paths, `_run_tool_request` dispatch, `_execute_tool_requests` error pass-through added in v21; `BasicLangGraphHand.stream()` (satisfied/max-iter/interrupt/file-changes/PR metadata) and `BasicAtomicHand.stream()` (satisfied/max-iter/interrupt/assertion-fallback/awaitable/PR metadata) added in v26 |
