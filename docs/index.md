...
- **Scheduled tasks (cron)**: `/schedules` CRUD endpoints manage recurring builds
  backed by RedBeat (Redis-based Celery Beat scheduler). Each schedule stores a
  cron expression, repo path, prompt, backend, and run options. Schedules can be
  enabled/disabled and manually triggered via `/schedules/{id}/trigger`. The
  `scheduled_build` Celery task delegates to `build_feature` on each cron tick.
  The built-in UI exposes a "Scheduled tasks" management view.
