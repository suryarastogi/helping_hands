...
import type { GardenPanelProps } from "./GardenPanel";
import type { WorldDecoration } from "../types";

const BASE_PROPS: GardenPanelProps = {
  maxWorkers: 8,
  activeWorkerCount: 0,
...
describe("GardenPanel component", () => {
  it("renders header and station/active counts", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} maxWorkers={12} activeWorkerCount={3} />);
    const summary = container.querySelector(".zen-status-summary");
    expect(summary).toBeTruthy();
    expect(summary?.textContent).toContain("Zen Garden");
    expect(summary?.textContent).toContain("12 Plots");
    expect(summary?.textContent).toContain("3 Gardening");
  });

  // --- Connection status ---

  it("shows multiplayer active hint when connected", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} connectionStatus="connected" />);
    expect(container.querySelector(".status-summary-hint")?.textContent).toContain("Multiplayer active");
  });

  it("shows disconnected hint", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} connectionStatus="disconnected" />);
    expect(container.querySelector(".status-summary-hint")?.textContent).toContain("Disconnected");
  });

  it("shows connecting hint", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} connectionStatus="connecting" />);
    expect(container.querySelector(".status-summary-hint")?.textContent).toContain("Connecting");
  });

  it("shows connection failed hint", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} connectionStatus="failed" />);
  ...
  it("renders 8 decoration emoji buttons", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} />);
  ...
  it("shows decoration count", () => {
    ...
    const { container } = render(<GardenPanel {...BASE_PROPS} decorations={decos} />);
  ...
  it("calls onSelectedDecoEmojiChange when emoji button clicked", () => {
    ...
    const { container } = render(<GardenPanel {...BASE_PROPS} onSelectedDecoEmojiChange={onChange} />);
  ...
  it("shows clear button when decorations exist", () => {
    ...
    const onClear = vi.fn();
    const { container } = render(<GardenPanel {...BASE_PROPS} decorations={decos} onClearDecorations={onClear} />);
  ...
  it("hides clear button when no decorations", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} decorations={[]} />);
    expect(container.querySelector(".decoration-clear-btn")).toBeNull();
  });

  it("shows placement hint when deco emoji is selected", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} selectedDecoEmoji="\u{1F338}" />);
  ...
  it("hides placement hint when no deco emoji selected", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} selectedDecoEmoji={null} />);
    expect(container.querySelector(".decoration-hint")).toBeNull();
  });

  it("disables decoration emoji buttons during cooldown", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} decoOnCooldown={true} />);
  ...
  it("enables decoration emoji buttons when not on cooldown", () => {
    const { container } = render(<GardenPanel {...BASE_PROPS} decoOnCooldown={false} />);
    const buttons = container.querySelectorAll(".decoration-emoji-btn");
    expect(buttons.length).toBeGreaterThan(0);
    buttons.forEach((btn) => {
