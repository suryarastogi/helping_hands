...
function versionPlugin(): Plugin {
  ...
  const replacement =
    `// Generated by hh-version plugin at vite startup.\n` +
    `export const FRONTEND_DISPLAY = ${JSON.stringify(fe.display)};\n` +
    `export const FRONTEND_LONG_SHA = ${JSON.stringify(fe.longSha)};\n` +
    `export const FRONTEND_COMMIT_DATE = ${JSON.stringify(fe.commitDate)};\n`;
  return {
    name: "hh-version",
    enforce: "pre",
    transform(_code, id) {
      if (id.replace(/\\/g, "/").endsWith("/src/version-generated.ts")) {
