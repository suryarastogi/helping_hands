...
- [x] **Safe CI gating** — integration test auto-runs dry-run off `master`; only `master` performs real PR updates
  ...
  - [x] **Claude CLI execution** — Subprocess-backed Claude Code flow with two phases, streaming output, npx fallback, non-interactive permissions, and enforcement retry for edit-intent prompts.
  ...
  - [x] **Gemini CLI execution** — Subprocess-backed Gemini flow with two phases, streaming output, auto-edit approval mode, and model-unavailable retry.
  - [x] **Backend selection/routing (full matrix)** — CLI and app-mode routing covers all backends: `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, `geminicli`, and `e2e`.
