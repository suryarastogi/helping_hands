...
- [x] **Safe CI gating** — integration test auto-runs dry-run off `master`; only `master` performs real PR updates
  - [x] **CLI hands implemented** — `ClaudeCodeHand`, `CodexCLIHand`, `GooseCLIHand`, and `GeminiCLIHand` backends in `src/helping_hands/lib/hands/v1/hand/cli/`.
  ...
  - [x] **Claude CLI execution** — Subprocess-backed `claude -p` flow with two phases, `npx` fallback, non-interactive permission skip, edit-enforcement retry.
  ...
  - [x] **Gemini CLI execution** — Subprocess-backed `gemini -p` flow with two phases, auto-edit approval mode, model-unavailable retry.
  - [x] **Goose CLI execution** — Subprocess-backed `goose run` flow with two phases, auto-derived `GOOSE_PROVIDER`/`GOOSE_MODEL`, `--with-builtin developer` injection.
  - [x] **Backend selection/routing (full matrix)** — CLI and app-mode routing covers all backends: `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, `geminicli`.
  - [x] **Streaming for CLI hands** — All CLI hands stream subprocess output incrementally with heartbeat/idle-timeout controls.
