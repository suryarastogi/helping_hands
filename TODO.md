...
- [x] **Safe CI gating** — integration test auto-runs dry-run off `master`; only `master` performs real PR updates
  - [x] **CLI hand scaffolds added** — `ClaudeCodeHand`, `CodexCLIHand`, `GooseCLIHand`, and `GeminiCLIHand` backends in `src/helping_hands/lib/hands/v1/hand/cli/`.
  ...
  - [x] **Backend selection/routing (CLI basic + E2E + all CLI)** — CLI supports `--backend {basic-langgraph,basic-atomic,basic-agent,codexcli,claudecodecli,goose,geminicli}` and `--e2e`.
  ...
  - [x] **Claude CLI execution** — Implemented subprocess-backed claude flow with two phases, npx fallback, root/sudo retry, no-change enforcement, and `--dangerously-skip-permissions` defaults.
  ...
  - [x] **Gemini CLI execution** — Implemented subprocess-backed gemini flow with two phases, `--approval-mode auto_edit` default, and model-unavailable retry.
  - [x] **Streaming for CLI hands** — All CLI hands use real incremental subprocess streaming via shared two-phase base class.
  - [x] **Cron-scheduled tasks** — App mode supports named cron schedules via RedBeat, with CRUD endpoints at `/schedules` and manual trigger support.
