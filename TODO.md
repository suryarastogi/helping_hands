...
- [x] **Safe CI gating** — integration test auto-runs dry-run off `master`; only `master` performs real PR updates
  ...
  - [x] **Claude CLI execution** — Implemented subprocess-backed claude flow with two-phase execution, streaming, `npx` fallback, root/sudo retry, no-change enforcement, and final PR integration.
  ...
  - [x] **Gemini CLI execution** — Implemented subprocess-backed gemini flow with two-phase execution, auto-edit approval mode, unavailable-model retry, and final PR integration.
  - [x] **Goose CLI execution** — Implemented subprocess-backed goose flow with two-phase execution, auto-injected `--with-builtin developer`, provider/model inference from `HELPING_HANDS_MODEL`, and final PR integration.
  - [x] **Backend selection/routing (full matrix)** — All backends (`basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, `geminicli`, `e2e`) are routed through CLI, app UI, and Celery task dispatch.
  - [x] **Streaming for CLI hands** — All CLI backends use real incremental subprocess streaming with heartbeat and idle-timeout guards.
  - [x] **Cron scheduling** — App mode supports cron-scheduled build tasks via RedBeat with CRUD API endpoints and named presets.
