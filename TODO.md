...
- [x] **Safe CI gating** — integration test auto-runs dry-run off `master`; only `master` performs real PR updates
  ...
  - [x] **Claude CLI execution** — Implemented subprocess-backed `claudecodecli` flow with two phases, streaming output, `npx` fallback, non-interactive permission mode, and enforcement retries.
  ...
  - [x] **Gemini CLI execution** — Implemented subprocess-backed `geminicli` flow with two phases, streaming output, model-unavailable retry, and `--approval-mode auto_edit` default.
  - [x] **Goose CLI execution** — Implemented subprocess-backed `goose` flow with two phases, auto-injected `--with-builtin developer`, provider/model derivation, and dual GitHub token mirroring.
  - [x] **Backend selection/routing (full matrix)** — CLI/server routing covers all backends: `basic-langgraph`, `basic-atomic`, `basic-agent`, `codexcli`, `claudecodecli`, `goose`, `geminicli`, and `e2e`.
  - [x] **Streaming for CLI hands** — All CLI backends use real incremental subprocess streaming with heartbeat and idle timeout support.
